...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    COALESCE((
      ...
      removed_primary AS (
        SELECT
          hsc.barangay_id
        FROM public.health_station_coverage hsc
        LEFT JOIN incoming incoming
          ON incoming.barangay_id = hsc.barangay_id
    ...
    COALESCE((
      ...
      assigned_primary AS (
        SELECT incoming.barangay_id
        FROM incoming
        WHERE incoming.is_active = true
          AND incoming.is_primary = true
      )
      SELECT jsonb_agg(
        jsonb_build_object(
          'barangay_id', b.id,
    ...
    COALESCE((
      ...
      projected AS (
        SELECT
          ...
          EXISTS (
            SELECT 1
            FROM public.health_station_coverage other
...
BEGIN
  ...
  LOOP
    ...
    IF v_row_id IS NULL THEN
      ...
      PERFORM public.append_audit_log(
        ...
        NULL,
        jsonb_build_object(
          'health_station_id', p_health_station_id,
  ...
  WHERE health_station_id = p_health_station_id
    AND is_active = true
    AND (
      cardinality(v_seen_barangay_ids) = 0
      OR barangay_id <> ALL(v_seen_barangay_ids)
    );

  RETURN jsonb_build_object(
    'processed_rows', v_processed,
    'primary_reassignments', v_conflicts,
...
BEGIN
  ...
  PERFORM public.append_audit_log(
    ...
    (
      SELECT to_jsonb(hs)
      FROM public.health_stations hs
      WHERE hs.id = p_health_station_id
...
BEGIN
  ...
  PERFORM public.append_audit_log(
    ...
    p_health_station_id,
    to_jsonb(v_station),
    to_jsonb(v_result) || jsonb_build_object('reason', p_reason)
  );
...
CREATE TRIGGER sync_health_station_legacy_fields_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.sync_health_station_legacy_fields();
...
CREATE TRIGGER ensure_health_station_legacy_coverage_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.ensure_health_station_legacy_coverage();
...
CREATE POLICY health_station_coverage_public_read
  ON public.health_station_coverage
  FOR SELECT
  USING (true);
...
FROM public.health_stations hs
JOIN public.city_barangays cb
  ON cb.id = hs.physical_city_barangay_id
LEFT JOIN public.health_station_coverage hsc
  ON hsc.health_station_id = hs.id
...
SELECT
  ...
  hsc.updated_at
FROM public.health_station_coverage hsc
JOIN public.health_stations hs
  ON hs.id = hsc.health_station_id
JOIN public.barangays b
  ON b.id = hsc.barangay_id
JOIN public.city_barangays cb
  ON cb.id = b.city_barangay_id;
